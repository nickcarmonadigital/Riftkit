...
**If installed manually** to `.agent/skills`:
...
```json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": ".agent/skills/continuous-learning-v2/hooks/observe.sh pre"
      }]
    }],
    "PostToolUse": [{
      "matcher": "*",
      "hooks": [{
        "type": "command",
        "command": ".agent/skills/continuous-learning-v2/hooks/observe.sh post"
      }]
    }]
  }
}
...
```bash
mkdir -p .agent/homunculus/{instincts/{personal,inherited},evolved/{agents,skills,commands}}
touch .agent/homunculus/observations.jsonl
```
...
```json
{
  ...
  "observation": {
    "enabled": true,
    "store_path": ".agent/homunculus/observations.jsonl",
    "max_file_size_mb": 10,
    "archive_after_days": 7
  },
  "instincts": {
    "personal_path": ".agent/homunculus/instincts/personal/",
    "inherited_path": ".agent/homunculus/instincts/inherited/",
    "min_confidence": 0.3,
  ...
  "evolution": {
    "cluster_threshold": 3,
    "evolved_path": ".agent/homunculus/evolved/"
  }
}
```
...
- Existing `.agent/skills/learned/` skills still work
