...
# New session call
Bash({
  command: ".agent/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend codex - \"$PWD\" <<'EOF'
...
# Resume session call
Bash({
  command: ".agent/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend codex resume <SESSION_ID> - \"$PWD\" <<'EOF'
ROLE_FILE: <role prompt path>
<TASK>
Requirement: <enhanced requirement (or $ARGUMENTS if not enhanced)>
Context: <project context and analysis from previous phases>
...
| Phase | Codex |
|-------|-------|
| Analysis | `.agent/.ccg/prompts/codex/analyzer.md` |
| Planning | `.agent/.ccg/prompts/codex/architect.md` |
| Review | `.agent/.ccg/prompts/codex/reviewer.md` |
