...
Manage Claude Code session history - list, load, alias, and edit sessions stored in `.agent/sessions/`.
...
console.log('Path: .agent/sessions/' + session.filename);
console.log('');
...
- Sessions are stored as markdown files in `.agent/sessions/`
- Aliases are stored in `.agent/session-aliases.json`
