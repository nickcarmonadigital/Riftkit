...
| **Auto-format** | `Edit` | Auto-formats JS/TS files with Biome or Prettier after edits |
...
Remove or comment out the hook entry in `hooks.json`. Override in your project or user settings:
...
All hooks use Node.js (`node -e` or `node script.js`) for maximum compatibility across Windows, macOS, and Linux. Avoid bash-specific syntax in hooks.
